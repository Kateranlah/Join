...
const colors = ["orange", "red", "green", "blue", "pink", "yellow", "ocean"];
...
function categoryIsEmpty() {
  return (
    document.getElementById("firstValue").innerText == "Select task Category"
  );
}
...
window.addEventListener("keypress", (event) => {
  ...
  const newCategory =
    document.getElementById("categorySelectorPlaceholder")
      ?.nextElementSibling || "";
...
function renderAssignedContactsIfClosed() {
  setTimeout(() => {
    if (
      document.getElementById("assignedSelector").offsetHeight > 50 ||
...
function renderInviteContactInput() {
  document.getElementById("assignedSelector").innerHTML =
    inviteContactInputTemplate();
...
function renderSingleContactFromArray(initials, color) {
  assignedToContainer.innerHTML += assignedToContactCircleTemplate(
    initials,
    color
  );
}
...
function resetInputRequiredMessages() {
  ...
  messagesAsArray.forEach((message) => (message.style.display = "none"));
  createTasktouched = false;
}
